async function getBasketballData(first_name, last_name) {
    try {
        ...
        const response = await fetch(`/api/football?first_name=${first_name}&last_name=${last_name}`);
        ...
        if (data.player && data.player.data && data.player.data.length > 0) {
            ...
            if (player) {
                ...
                document.getElementById('FBresultContainer').innerHTML = "<p>No player data found for the provided name.</p>";
            }
        } else {
            console.log("No data found in the API response.");
            document.getElementById('FBresultContainer').innerHTML = "<p>No player data found.</p>";
        }

    } catch (error) {
        console.error('Error:', error);
        document.getElementById('FBresultContainer').innerHTML = `<p>Error: ${error.message}</p>`;
    }
}

function displayPlayerData(player) {
    const resultContainer = document.getElementById('FBresultContainer');
    
    
    resultContainer.innerHTML = `

    <div class="FBresultContainer">
